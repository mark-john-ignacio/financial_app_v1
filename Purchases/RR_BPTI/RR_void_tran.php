<?php
	...
	$_SESSION['pageid'] = "Receive_unpost.php";
	...
	$status = "True";
			...
			if (!mysqli_query($con,"Update receive set lvoid=1 where compcode='$company' and ctranno in ('".implode("','",$_POST["allbox"])."')")){
				$status = "False";	
			}else{
				...
				foreach($_POST["allbox"] as $rz){
					...
					values('$company','$rz','$preparedby',NOW(),'VOID','RECEIVING','$compname','Void Record','".$_POST["hdnreason"]."')");
...
?>
				...
				<script>
					...
					window.location.href="RR_void.php";
				</script>
<?php
			}else{
?>
