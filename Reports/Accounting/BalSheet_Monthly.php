...
<br><br>
<table border="0" align="center" cellpadding="3" class="my-table">
  <tr>
...
?>
   <tr>
    ...
    <td onclick="funcset('<?=$row['cacctid']?>','<?=$date1?>','<?=$date2?>')" style="cursor: pointer" nowrap>
			<?php
				if($row['ctype']=="General"){
					//echo "<b>".$row['cacctid']."</b>";
				//	echo $row['nlevel'].":".$arrlvlcnt;
				}else{
					echo $row['cacctid'];
				}
			?>
		</td>
    <td onclick="funcset('<?=$row['cacctid']?>','<?=$date1?>','<?=$date2?>')" style="padding-right: 20px; cursor: pointer; text-indent:<?=$GENxyz1?>px " nowrap>
			<?php
				if($row['ctype']=="General"){
				...
				<?php
					if($row['ctype']=="Details"){
						...
						if($row['cacctid']==$acct_retained){
							$xcvb11 = gettotal($row['cacctid'], $row['ccategory'], $rxzm);

							$tagwithRetained = 1;
							$xcvb = $RetainedGross[$rxzm] + $xcvb11;
						}else{
...
<tr>
	...
	<?php
		foreach($hdr_months as $rxzm){
			...
			$toformat = (floatval($twinGross[$rxzm])<0) ? "(".number_format(abs($twinGross[$rxzm]),2).")": number_format($twinGross[$rxzm],2);
	?>
