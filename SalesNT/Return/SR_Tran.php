...
if($_REQUEST['typ']=="POST"){
	...
	if (!mysqli_query($con,"Update ntsalesreturn set lapproved=1 where compcode='$company' and ctranno='$tranno'")) {
	...
	values('$tranno','$preparedby',NOW(),'POSTED','SR Non-Trade','$compname','Post Record')");

}

if($_REQUEST['typ']=="CANCEL"){

	if (!mysqli_query($con,"Update ntsalesreturn set lcancelled=1 where compcode='$company' and ctranno='$tranno'")) {
...
mysqli_query($con,"INSERT INTO logfile(`ctranno`, `cuser`, `ddate`, `cevent`,`module`, `cmachine`, `cremarks`) 
	values('$tranno','$preparedby',NOW(),'CANCELLED','SR Non-Trade','$compname','Cancel Record')");
