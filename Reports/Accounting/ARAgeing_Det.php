...
$arrsupps = array();
	...
	$arrsuppx = "";
	$arrterms = "";
	$sqlpay = "Select A.cempid as ccode, COALESCE(A.ctradename,A.cname) as cname, A.cterms, IFNULL(B.nintval,0) as nintval from customers A left join groupings B on A.compcode=B.compcode and A.cterms=B.ccode and B.ctype='TERMS' Where A.compcode='$company' and A.cempid = '".$ccodex ."'";
  ...
  <tr>
	...
	<?php
		...
		if($salestat!==""){
			...
			left JOIN
				(
					...
					From aradjustment
					Where compcode='$company' and lapproved=1 and isreturn=1 and ccode='".$ccodex ."'
					Group by crefsi
			...
			left JOIN
				(
					Select crefsi, Sum(ngross) as ncredit
					From aradjustment
					Where compcode='$company' and lapproved=1 and ccode='".$ccodex ."'
					Group by crefsi
				) B on A.ctranno=B.crefsi
			left JOIN
				(
		...
		while($row999 = mysqli_fetch_array($result, MYSQLI_ASSOC))
		{
			...
			$sinet = floatval($row999['ngross']) - floatval($row999['ncredit']);

			if($sinet > floatval($row999['napplied'])){
