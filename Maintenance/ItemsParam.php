...
$_SESSION['pageid'] = "ItemsParam.php";
...
<html>
  ...
  ?>
<form class="form-inline" role="form" method="post" action="ItemsParam_Save.php" id="frmparam" name="frmparam">
...
$company = $_SESSION['companyid'];
  ...
  $sql = "Select * from groupings where compcode='$company' and ctype='$typ' Order By ccode";
...
<tr>
    <td width="30%">
    <div class="col-xs-10">
    <input type="text" readonly name="nord<?php echo $cntr?>" id="nord<?php echo $cntr?>" value="<?php echo $row['ccode'];?>" class="form-control input-group-sm">
    </div>
    </td>
    <td><input type="text" name="nvalz<?php echo $cntr?>" id="nvalz<?php echo $cntr?>" value="<?php echo $row['cdesc'];?>" class="form-control input-group-sm"></td>
    <td width="10%">
...
function chkform(){
	...
	if(lastRow==0){
		...
		for (z=1; z<=lastRow; z++){
			if(document.getElementById("nvalz"+z).value=="" || document.getElementById("nord"+z).value==""){
				withval = "NO";
			}
		}
	}
...
function addval(){
	...
	w.style.width = "10%";
	u.innerHTML = "<div class=\"col-xs-10\"><input type='text' name='nord"+lastRow+"' id='nord"+lastRow+"' class='form-control input-group-sm'> </div>";
