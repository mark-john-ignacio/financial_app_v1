...
<body style="padding:5px">
        ...
        <br><br>
				...
				<table id="example" class="display" cellspacing="0" width="100%" text-align="center">
					<thead>
						<tr>
							<th width="100">Log ID</th>
							<th>User ID</th>
                            <th width="80">Date</th>
							<th width="80">Online</th>
                            <th width="80">Time in</th>
							<th width="80">Offline</th>
						...
						?>
							<tr>
								...
								<td><?php echo $row['Userid'];?>
									<div class="itmalert alert alert-danger nopadding" id="itm<?php echo $row['Userid'];?>" style="display: inline";></div>
								</td>
								<td>
								...
								<td>
									<input class='btn btn-danger btn-xs' type='button' id='row_<?php echo $row['ccode'];?>_delete' value='delete' onClick="deleteRow('<?php echo $row['ccode'];?>');"/>
								</td>
							</tr>
						<?php 
							}				
		...
		<div class="modal-dialog modal-md">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="myModalLabel"><b>Add New UOM</b></h5>        
				</div>
  ...
  <link rel="stylesheet" type="text/css" href="../../Bootstrap/DataTable/DataTable.css"> 
	...
	$(function(){
		...
		$("#btnadd").on("click", function() {
		 ...
		 if(x.trim()=="True"){
			...
			$('#myModalLabel').html("<b>Add New UOM</b>");
			$('#myModal').modal('show');
		 } else {
		...
		$("#btnSave, #btnUpdate").on("click", function() {
			...
			if(varcode=="" || vardesc==""){
				...
				$.ajax ({
					url: "th_saveuom.php",
					data: { desc: vardesc, code: varcode, typ: ctype },
					async: false,
					success: function( data ) {
						if(data.trim()=="True"){
	...
	function editgrp(code,desc){
		 ...
		 if(x.trim()=="True"){
			...
			$('#myModalLabel').html("<b>Update UOM Detail</b>");
			$('#myModal').modal('show');
		 } else {
			 $("#AlertMsg").html("<center><b>ACCESS DENIED!</b></center>");
			 $("#AlertModal").modal('show');
		...
		function setStat(code, stat){
			$.ajax ({
				...
				data: { code: code,  stat: stat, typz: 'ITMUNIT' },
				async: false,
				success: function( data ) {
					if(data.trim()!="True"){
