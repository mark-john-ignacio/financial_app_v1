...
<form action="ARAdj_newsave.php" name="frmpos" id="frmpos" method="post" onSubmit="return false;">
  ...
  <tr>
    ...
    <td>       
    <div class="col-xs-12 nopadding">
        	<div class="col-xs-3 nopadding"><input type="text" class="form-control input-sm" id="txtSIRef" name="txtSIRef" width="20px" tabindex="2" readonly placeholder="Search Invoice No...">
            </div>
         ...
         <div class="alt2" dir="ltr" style="
            ...
            <table id="MyTable" class="MyTable table table-condensed" width="100%">
					...
					<tr>
                        ...
                        <th style="border-bottom:1px solid #999">Item No.</th>
...
<!-- SI Reference Details -->

<div class="modal fade" id="SIRefDet" role="dialog" data-keyboard="false" data-backdrop="static">
    <div class="modal-dialog modal-lg" style="width:130vh">
        <div class="modal-content">
            <div class="modal-header">
            	<button type="button" class="close" data-dismiss="modal" aria-label="Close" id="btnCloseDetRef"><span aria-hidden="true">&times;</span></button>
                <h3 class="modal-title" id="InvListDet">SI Details</h3>
                <input type="hidden" name="hdntmpref" id="hdntmpref"> 
            </div>
            
            <div class="modal-body" style="height:40vh">
                    <div class="col-xs-12 nopadding pre-scrollable" style="height:37vh">
                          <table name='MyInvDet' id='MyInvDet' class="table table-condensed">
                           <thead>
                            <tr>
                              <th rowspan="2">Item No.</th>
                              <th rowspan="2">Description</th>
                              <th rowspan="2">UOM</th>
                              <th colspan="3" class="text-center">Original</th>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-success" id="OKRefDet" onClick="computeDet()">INSERT ACCOUNT DETAILS</button>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
</div><!-- /.modal -->
...
function InsertWOR(){
	...
	var tdinv= "<td><input type='text' class='form-control input-xs' name='txtcinvno' id='txtcinvno"+lastRow+"' value='WOR' readonly></td>";
...
function computeDet(){
	var refSI = $("#hdntmpref").val();
	var detitms = "";
	var cnt = 0;
	
			$("#MyInvDet > tbody > tr").each(function() {
				var itm = $(this).find('input[type="hidden"][name="txtitmref"]').val();
	...
	var tbl = document.getElementById('MyInvDet').getElementsByTagName('tr');
	var lastRow = tbl.length;
	
	var tdinv= "<td><input type='text' class='form-control input-xs' name='txtcinvno' id='txtcinvno"+lastRow+"' value=\""+refSI+"\" ></td>";
	var tdaact = "<td><input type='text' class='cacctno form-control input-xs' name='txtcacctno' id='txtcacctno"+lastRow+"' placeholder=\"Enter Acct Code...\"></td>";
	var tdtitle = "<td width='230'><input type='text' class='acctdesc form-control input-xs' name='txtctitle' id='txtctitle"+lastRow+"' placeholder=\"Enter Acct Title...\"></td>";
	var tddesc = "<td width='200'><input type='text' class='form-control input-xs' name='txtcdesc' id='txtcdesc"+lastRow+"' placeholder=\"Enter Description...\"></td>";
	var tddebit = "<td><input type='text' class='numeric form-control input-xs' name='txtndebit' id='txtndebit"+lastRow+"' style=\"text-align:right\" value=\"0.0000\"></td>";
...
function setinvref(invref){
	...
	$('#MyInvDet tbody').empty();
	var adjtyp = $("#seltype").val();
									
		if(adjtyp=="Credit"){
			$("#qtyadjtxt").html("Qty (Deduct)");
		}
		else if(adjtyp=="Debit"){
			$("#qtyadjtxt").html("Qty (Add)");
		}
	
	
	// $("#txtSIRef").val(invref); 
	$("#hdntmpref").val(invref);
	 $('#InvListDet').html("Invoice Details: " + invref);
	 
			$.ajax({
                    url: 'th_invrefdet.php',
					data: 'x='+invref,
                    dataType: 'json',
                    method: 'post',
					async: false,
                    success: function (data) {
                       ...
                       $.each(data,function(index,item){
							...
							$("<tr>").append(
							$("<td>").html("<input type='hidden' name='txtitmref' id='txtitmref"+index+"' value=\""+item.citemno+"\">" + item.citemno),
							$("<td>").text(item.cdesc),
							$("<td>").text(item.cunit),
							$("<td>").html("<input type='hidden' name='txtnorigref' id='txtnorigref"+index+"' value=\""+item.nqty+"\">" + item.nqty),
							$("<td>").text(item.nprice),
							$("<td>").text(item.namount),
							$("<td>").html("<input type='text' class='numeric form-control input-xs' name='txtnqtyref' id='txtnqtyref"+index+"' style=\"text-align:right\" value=\"0.0000\">"),
							$("<td>").html("<input type='text' class='numeric form-control input-xs' name='txtnprcref' id='txtnprcref"+index+"' style=\"text-align:right\" value=\""+item.nprice+"\">"),
							$("<td>").html("<input type='text' class='numeric form-control input-xs' name='txtnamtref' id='txtnamtref"+index+"' style=\"text-align:right\" value=\"0.0000\" readonly>")
							).appendTo("#MyInvDet tbody");
								...
								$("#txtnqtyref"+index).on("keyup", function() {
									var xy = $(this).val();
									var yz = $("#txtnprcref"+index).val();
									
									var xyorig = $("#txtnorigref"+index).val();
									
										if(adjtyp=="Credit"){
												if(parseFloat(xy) > parseFloat(xyorig)){
													$(this).val(xyorig);
									...
									var zamt = parseFloat(xy) * parseFloat(yz);
									$("#txtnamtref"+index).val(zamt.toFixed(4));
...
function PopReturns(){
	...
	$('#InvListHdr').html("Invoice List: " + $('#txtcust').val());
			...
			$("#MyTable > tbody > tr").each(function() {
				myxref = $(this).find('input[type="hidden"][name="txtcsrno"]').val();
				  ...
				  if(cnt>1){
					...
					salesnos = salesnos +  $(this).find('input[type="hidden"][name="txtcsrno"]').val();
                    ...
                    success: function (data) {
                       ...
                       $.each(data,function(index,item){
						...
						$("#AlertModal").modal('show');
							...
							$("<tr>").append(
							$("<td id='td"+item.cpono+"'>").html("<a href=\"#SIRefDet\" data-toggle=\"modal\" data-dismiss=\"modal\" onclick=\"setinvref('"+item.cpono+"')\">"+item.cpono+"</a>"),
							$("<td>").text(item.ngross),
