...
<small><i>*Press tab after remarks field (last row) to add new line..</i></small>
             ...
             <div class="alt2" dir="ltr" style="
               ...
               <tbody class="tbody">
                    ...
                    <?php 
						$sqlbody = mysqli_query($con,"select a.* from journal_t a where a.compcode='$company' and a.ctranno = '$cjeno' order by a.nidentity");
...
function chkform(){
	...
	for (i = 1; i <= cnt; i++) {
		if($("#txtcAcctNo"+i).val() == "" || $("#txtcAcctDesc"+i).val() == ""){
			alert("Valid Account ID and Description is required!");
			$("#txtcAcctNo"+i).focus();
		...
		if($("#txtnCredit"+i).val()==0 && $("#txtnDebit"+i).val()==0){
			alert("Input Debit or Credit amount for this row!");
			$("#txtnDebit"+i).focus();
