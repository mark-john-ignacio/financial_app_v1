...
function chkgrp($valz) {
	...
	if (!mysqli_query($con,"INSERT INTO sales_t(`compcode`, `cidentity`, `ctranno`, `creference`, `nident`, `citemno`, `nqty`, `cunit`, `nprice`, `namount`, `cmainunit`,`nfactor`,`cacctcode`,`ctaxcode`) values('$company', '$refcidenttran', '$cSINo', $crefno, '$indexz', '$cItemNo', '$nQty', '$cUnit', '$nPrice', '$nAmount', '$cMainUOM', $nFactor, $cacctcode, $ctaxcode)")){
		//echo "False";
		...
		//Compute Tax per item if company is set as VATABLE ... 
				...
				if($xcomp==1){ // pag for compute ang VAT ng mismong company check naman ung customer
					...
					$sqlhead = mysqli_query($con,"Select A.cvattype, B.lcompute from customers A left join vatcode B on A.compcode=B.compcode and A.cvattype=B.cvatcode where A.compcode='$company'");
					if (mysqli_num_rows($sqlhead)!=0) {
						$row = mysqli_fetch_assoc($sqlhead);
						...
						if($xycomp == 1){
							...
							if($nrate!=0){
								...
								mysqli_query($con,"UPDATE sales_t set nnetvat=$varnetvat, nlessvat=$varlessvat where compcode='$company' and ctranno='$cSINo'");
