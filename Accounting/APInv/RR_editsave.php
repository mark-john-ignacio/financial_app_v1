...
$company = $_SESSION['companyid'];
	...
	if (!mysqli_query($con,"Update suppinv set `ccode` ='$cCustID', `cremarks`=$cRemarks, `dreceived`=STR_TO_DATE('$dDelDate', '%m/%d/%Y'),`ngross`='$nGross', `nbasegross` = '$BaseGross', `ccurrencycode` = '$CurrCode', `ccurrencydesc` = '$CurrDesc', `nexchangerate` = '$CurrRate', `ccustacctcode`='$AccntCode', `lapproved` = 0, `crefsi` = '$cCustSI' Where compcode='$company' and ctranno='$cSINo'")){
		echo "False";
	}
	else{
		echo $cSINo;
