...
<table width="100%" border="0" cellpadding="3" style="border-collapse:collapse;" id="tblMain">
  ...
  <tr>
    ...
    <table width="100%" border="0" cellpadding="3" cellspacing="5">
      <tr>
        ...
        <td width="40%" height="60" valign="top" style="border:1px solid; border-style:dashed;"><font size="2"><b>DELIVERY DETAILS:</b></font><br>&nbsp;&nbsp; &nbsp; <?php echo $Remarks;?>
        
        <?php
        	if($lPrintPosted==1){
				echo "<font color='#FF0000'><b><i>ORIGINAL PR ALREADY PRINTED<i></b></font>";
			}
		?>
        </td>
      </tr>
  ...
  <tr>
    ...
    <table width="100%" border="0" cellpadding="3" style="border-style:dashed;">
      ...
      <?php 
		$sqlbody = mysqli_query($con,"select a.*,b.citemdesc from purchreturn_t a left join items b on a.citemno=b.cpartno and a.compcode=b.compcode where a.compcode='$company' and a.ctranno = '$cpono'");

		if (!mysqli_query($con, "select a.*,b.citemdesc from purchreturn_t a left join items b on a.citemno=b.cpartno where a.compcode='$company' and a.ctranno = '$cpono'")) {
...
//POST RECORD
if($lPrintPosted==0){
mysqli_query($con,"Update purchreturn set lprintposted=1, lapproved=1 where compcode='$company' and ctranno='$cpono'");

//WRREntry($cpono);
//ToInv($cpono,"WRR","IN",$Date);

//Update items table cost, retailcost and stockonhands
		$UpdateItem = mysqli_query($con,"Select A.citemno, B.citemdesc, A.nqty as nqtyin, A.nfactor as nfactorin, A.ncost, A.nretail, B.nqty, B.npurchcost, B.nretailcost From purchreturn_t A left join items B on A.compcode=B.compcode and A.citemno=B.cpartno Where A.compcode='$company' and A.ctranno='$cpono'");
