        ...
        <div class="modal-content">
            ...
            <div class="modal-body" style="height:20vh">
				...
				<div class="row">
					...
					<div class="col-xs-1 nopadwtop" id="htmlserqtyneed">
						<input type="hidden" name="hdnserqtyneed" id="hdnserqtyneed">
					</div>
					<div class="col-xs-1 nopadwtop" id="htmlserqtyuom">
						<input type="hidden" name="hdnserqtyuom" id="hdnserqtyuom">
					</div>
				</div>
...
<script type="text/javascript">
	...
	function myFunctionadd(nqty,nprice,namount,nfactor,cmainunit,xref,nident){
		...
		$('#MyTable > tbody:last-child').append(
			"<tr>"
			+"<td width='50px'><input class='btn btn-info btn-xs' type='button' name='btnins' id='ins" + lastRow + "' value='insert' data-itr=\""+lastRow+"\"/></td>"
			...
			+"<td width='120px' nowrap style='padding:1px;overflow: hidden;text-overflow: ellipsis;'><input type='hidden' value='"+itmdesc+"' name=\"txtitemdesc\" id=\"txtitemdesc\">"+itmdesc+"</td>"
			+"<td width='80px' style='padding:1px'><input type='hidden' value='"+itmunit+"' name=\"seluom\" id=\"seluom\">"+itmunit+"</td>"
			...
			+"<td width='80px' style='padding:1px'><input class='btn btn-danger btn-xs' type='button' name='btndel' id='del" + lastRow + "' value='delete' data-itx=\""+lastRow+"\"/></td>"
		);								

		$("#del"+lastRow).on('click', function() {
			$("#MyTable2 > tbody > tr").each(function(index) {						
				if($(this).find('input[type="hidden"][name="sertabident"]').val()==$(this).data('itx')){
		...
		$("#ins"+lastRow).on('click', function() {
			var xa = $(this).closest("tr").find("input[type=\"hidden\"][name=txtitemcode]").val();
			var xb = $(this).closest("tr").find("input[type=\"hidden\"][name=txtitemdesc]").val();
			var xc = $(this).closest("tr").find("input[type=\"hidden\"][name=seluom]").val();
			var xd = $(this).closest("tr").find("input[type=\"hidden\"][name=hdnfactor]").val();
			var xe = $(this).closest("tr").find("input[type=\"hidden\"][name=hdnmainuom]").val(); 
			var xf = $(this).closest("tr").find("input[type=\"hidden\"][name=txtcreference]").val();
			var xcsd = $(this).closest("tr").find("input[name=txtnqty]").val();

			$("#serdisrefident").val($(this).data("itr"));

			InsertDetSerial(xa, xb, xc, xd, xcsd, xd, xe, xf);
		});
	...
	function delreindex(){
		$("#MyTable > tbody > tr").each(function(index) {	
			...
			$(this).find('input[type="button"][name="btnins"]').attr("id","ins"+$xv);
			$getitr = (this).find('input[type="button"][name="btnins"]').data("itr");
				$("#MyTable2 > tbody > tr").each(function(index) {						
					if($(this).find('input[type="hidden"][name="sertabident"]').val()==$getitr){

					}
				});

			$(this).find('input[type="button"][name="btnins"]').data("itr",$xv);

			$(this).find('input[name="txtnqty"]').attr("id","txtnqty"+$xv);
			$(this).find('input[type="hidden"][name="txtnqtyORIG"]').attr("id","txtnqtyORIG"+$xv);
			$(this).find('input[type="hidden"][name="txtnprice"]').attr("id","txtnprice"+$xv);
			$(this).find('input[type="hidden"][name="txtnamount"]').attr("id","txtnamount"+$xv);
			$(this).find('input[type="hidden"][name="hdnmainuom"]').attr("id","hdnmainuom"+$xv);
			$(this).find('input[type="hidden"][name="hdnfactor"]').attr("id","hdnfactor"+$xv);
			$(this).find('input[name="dremarks"]').attr("id","dremarks"+$xv);
			$(this).find('input[type="button"][name="btndel"]').attr("id","del"+$xv);
		...
		$.ajax({
			...
			{	
				...
				$.each(data,function(index,item){ 
					...
					$("<tr>").append(
						$("<td>").html("<input type='hidden' value='"+itmcode+"' name=\"lagyitmcode\" id=\"lagyitmcode\"><input type='hidden' value='"+item.clotsno+"' name=\"lagylotno\" id=\"lagylotno\"><input type='hidden' value='"+item.nrefidentity+"' name=\"lagyrefident\" id=\"lagyrefident\"><input type='hidden' value='"+item.ctranno+"' name=\"lagyrefno\" id=\"lagyrefno\">"+item.clotsno), 
						$("<td width=\"100px\" style=\"padding:1px\">").html("<input type='hidden' value='"+item.cpacklist+"' name=\"lagypacklist\" id=\"lagypacklist\">"+item.cpacklist),
						...
						$("<td width=\"80px\" style=\"padding:1px\">").html("<input type='hidden' value='"+item.cunit+"' name=\"lagycuom\" id=\"lagycuom\">"+item.mainunit),
		...
		var isDone = "True";
			...
			if(trancode!=""){
				...
				$("#MyTable2 > tbody > tr").each(function(index) {
					...
					var xcrefid = $(this).find('input[type="hidden"][name="sertabreferid"]').val(); 
					var xcref = $(this).find('input[type="hidden"][name="sertabrefer"]').val();
					var xlotsno = $(this).find('input[type="hidden"][name="sertablotsno"]').val(); 	
					var xpckslist = $(this).find('input[type="hidden"][name="sertabpackno"]').val();	
					...
					$.ajax ({
						...
						data: { trancode: trancode, indx: index, citmno: citmno, cuom: cuom, nqty:nqty, clocas:clocas, xcrefid:xcrefid, xcref:xcref, crefidnt:crefidnt, xlotsno:xlotsno, xpckslist:xpckslist },
						async: false,
						success: function( data ) {
							if(data.trim()=="False"){
								isDone = "False";
