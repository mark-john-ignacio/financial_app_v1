...
<table width="100%">
<tr>
	<td align="left"><b>Receiving Listing</b><input type="hidden" name="hdnSI" id="hdnSI" value="<?php echo $_REQUEST['id']?>" /></td>
    <td align="right"><input type="button" value="OK"  onClick="inserttotbl();">
...
$sql = "select * from receive where lapproved=1 and ccode='$salesno' and ctranno not in (Select crefno from apv_d) ".$qry;
...
?>
    ...
    <tr bgcolor="<?php echo $bg; ?>" id="nyca<?php echo $ctr;?>">
      <td><input id="chkitem<?php echo $ctr;?>" name="chkitem[]" type="checkbox" value="<?php echo $ctr; ?>" onclick="javascript:chk('<?php echo $ctr;?>','<?php echo $bg; ?>')" class="messageCheckbox">&nbsp;&nbsp;<?php echo $row['ctranno']; ?>
       ...
       <input type="hidden" name="tranno<?php echo $ctr;?>" id="tranno<?php echo $ctr;?>" value="<?php echo $row['ctranno']; ?>">
      </td>
      <td><?php echo $row['dreceived']; ?></td>
      <td><?php echo $row['ngross']; ?></td>
      <td align="right"><?php echo $row['cremarks']; ?></td>
    </tr>
