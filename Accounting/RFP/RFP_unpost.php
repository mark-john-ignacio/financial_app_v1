...
<body style="padding:5px">
	<form action="RFP_unpost_tran.php" name="frmunpost" id="frmunpost" method="POST">
	
		<div>
			<section>
					<div>
						<div style="float:left; width:50%">
							<font size="+2"><u>Request For Payment List</u></font>	
						</div>
					</div>
				<br><br>
              ...
              <th>AP Gross</th>
					...
					<tbody>
					<?php
					$alrr = mysqli_query($con,"Select a.capvno from paybill_t a left join paybill b on a.compcode=b.compcode and a.ctranno=b.ctranno where a.compcode='$company' and b.lcancelled=0");
					$refpos[] = "";
					while($rowxcv=mysqli_fetch_array($alrr, MYSQLI_ASSOC)){
						$refpos[] = $rowxcv['capvno'];
					}

					//all rfp_t with APV no. not in paybill

					$refrfp[] = "";
					$alrr = mysqli_query($con,"Select ctranno from rfp_t where compcode='$company' and capvno not in ('".implode("','",$refpos)."')");
					while($rowxcv=mysqli_fetch_array($alrr, MYSQLI_ASSOC)){
						$refrfp[] = $rowxcv['ctranno'];
					}

					$result=mysqli_query($con,"select a.*, b.cname from rfp a left join suppliers b on a.compcode=b.compcode and a.ccode=b.ccode where a.compcode='$company' and a.ctranno in ('".implode("','",$refrfp)."') and (a.lapproved=1 or a.lcancelled=1) order by a.ddate desc");
...
<script type="text/javascript">
	...
	function printchk(x){
		$("#myprintframe").attr("src","RFP_print.php?x="+x);
