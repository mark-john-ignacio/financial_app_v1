...
$company = $_SESSION['companyid'];
	...
	if (!mysqli_query($con, "UPDATE `apadjustment` set `ccode` = '$cCustID', `dcutdate` = STR_TO_DATE('$dTranDate', '%m/%d/%Y'), `ctype` = '$cSelType', `cremarks` = '$cRemarks', `ngross` = '$ngross', `crefsr` = '$cSRRef', `crefsi` = '$cSIRef', `isreturn` = '$dret', `ccurrencycode` = '$cCurrCode' where `compcode`='$company' and `ctranno` = '$cSINo'")) {
	...
	if(file_exists('../../Components/assets/AP-ADJ/'.$company.'_'.$cSINo.'/')) {
		...
		if($total_count>=1){
			mkdir('../../Components/assets/AP-ADJ/'.$company.'_'.$cSINo.'/',0777);
		}
	}
	...
	for( $i=0 ; $i < $total_count ; $i++ ) {
		...
		if ($tmpFilePath != ""){
				...
				$newFilePath = "../../Components/assets/AP-ADJ/" .$company.'_'. $cSINo . "/" . $_FILES['upload']['name'][$i];
