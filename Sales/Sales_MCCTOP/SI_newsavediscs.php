...
//trancode: trancode, discnme: discnme, seldisctyp: seldisctyp, cinfofld: discval, discamt: discamt, discacctno: discacctno
		...
		if (!mysqli_query($con,"INSERT INTO sales_t_disc(`compcode`, `cidentity`, `nident`, `ctranno`, `citemnoident`, `citemno`, `discounts_list_code`, `cdisctype`, `nvalue`, `namount`, `cacctno`) values('$company', '$refcidenttran', '$indexz', '$cSINo', '$discitmnoident', '$discitmno', '$discnme', '$seldisctyp', '$discval', '$discamt', '$discacctno')")){
			echo "False";
		}
		else{
			echo "True";
