<?php
  ...
  $arrdneed1 = "";
	$sqlempsec = mysqli_query($con,"select dneeded FROM purchrequest where compcode='$company' Order By dneeded ASC LIMIT 1");
	$rowdetloc = $sqlempsec->fetch_all(MYSQLI_ASSOC);
	foreach($rowdetloc as $row0){
		$arrdneed1 = $row0['dneeded'];
	}
...
<center>
  <font size="+1"><b><u>Documents Analysis Purchase Request Vs Purchases Orders</u></b></font>
</center>
<br>

<form action="Purchases/PRvsPO.php" method="post" name="frmrep" id="frmrep" target="_blank">
        ...
        <div class="form-group nopadding">
            ...
            <div class="input-group input-large date-picker input-daterange">
                <?php
                      $xcd = "";
                      if($arrdneed1!=""){
                        $date=date_create($arrdneed1);
                        $xcd = date_format($date,"m/d/Y");

                      }else{
                        $xcd = date("m/d/Y");
                      }
                    ?>
                    <input type="text" class="datepick form-control input-sm" id="date1" name="date1" value="<?php echo $xcd; ?>">
  ...
  <tr>
    ...
    <td style="padding:2px">
      <div class="col-xs-8 nopadding">
          ...
          <option value="">ALL Sections</options>
          <?php
            ...
            while($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
					  {
          ?>
              <option value="<?php echo $row['nid'];?>"><?php echo $row['cdesc'];?></options>
          <?php
            }
          ?>
        </select>
...
$(function(){
    ...
    $('#btnView').on("click", function(){
      $('#frmrep').attr("action", "Purchases/PRvsPO.php");
      $('#frmrep').submit();
    });

    $('#btnexcel').on("click", function(){
      $('#frmrep').attr("action", "Purchases/PRvsPO_xls.php");
      $('#frmrep').submit();
    });
});
</script>