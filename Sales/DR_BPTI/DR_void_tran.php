<?php
	...
	if (!mysqli_query($con,"Update dr set lvoid=1 where compcode='$company' and ctranno in ('".implode("','",$_POST["allbox"])."')")){
		$status = "False";	
	}else{
		...
		foreach($_POST["allbox"] as $rz){
			...
			values('$company', '$rz','$preparedby',NOW(),'VOID','DELIVERY RECEIPT','$compname','Void Record','".$_REQUEST['hdnreason']."')");
