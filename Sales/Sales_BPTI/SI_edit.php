	...
	<form action="SI_edit.php?hdnsrchval=<?=(isset($_REQUEST['hdnsrchval'])) ? $_REQUEST['hdnsrchval'] : ""?>" name="frmpos" id="frmpos" method="post">
		<div class="portlet">
			<div class="portlet-title">
				<div class="caption">
					<i class="fa fa-file-text"></i>Sales Invoice Details
				</div>
				<div class="status">
					<?php
						if($lCancelled==1){
					...
					<div id="home" class="tab-pane fade in active" style="padding-left: 5px; padding-top: 10px">
						...
						<table width="100%" border="0">
							...
							<tr>
								...
								<tH width="100"><b>Oracle Invoice</b></tH>
								<td style="padding:2px;">
									<div class="col-xs-11 nopadding">
										<input type='text' class="form-control input-sm" id="coracleinv" name="coracleinv" value="<?=$oracleSI?>" autocomplete="off"/>
									</div>
								</td>
							</tr>
					...
					<div class="portlet-title">
						...
						<div class="inputs">
							...
							<input type="hidden" name="hdnqty" id="hdnqty">
								...
								<div class="col-xs-12 nopadding">
									<div class="chkitmsadd col-xs-4 nopadwdown">
				...
				<div class="modal-body" style="height:40vh">
					...
					<div class="col-xs-12 nopadding">
						...
						<div class="form-group">
							<div class="col-xs-4 pre-scrollable" style="height:37vh; border-right: 2px solid #ccc">
							...
							<div class="col-xs-8 pre-scrollable" style="height:37vh; border-right: 2px solid #ccc">
								<table name='MyInvDetList' id='MyInvDetList' class="table table-small small">
									<thead>
										<tr>
											...
											<th nowrap>SO No.</th> 
											<th nowrap>Item No</th>
											<th nowrap>Description</th>
											<th nowrap>UOM</th>
											<th nowrap>Qty</th>
											<th nowrap style="text-align: center">Price</th>
						...
						<div class="form-group">
							<div class="col-xs-4 nopadding pre-scrollable" style="height:37vh">
								<table name='MyQOTbl' id='MyQOTbl' class="table table-small table-highlight small">
									<thead>
										<tr>
											<th nowrap id="QOTBLLabel">Bill No</th>
							...
							<div class="col-xs-8 nopadwleft pre-scrollable" style="height:37vh">
								<table name='MyQODetList' id='MyQODetList' class="table table-small small">
									<thead>
										<tr>
											<th nowrap style="text-align: center"> <input name="allboxqo" id="allboxqo" type="checkbox" value="Check All" /></th>
											<th nowrap>Item No</th>
											<th nowrap>Description</th>
											<th nowrap>UOM</th>
											<th nowrap>Qty</th>
...
function myFunctionadd(qty,pricex,ndisc,curramt,amtx,factr,cref,nrefident,citmcls,cvat){
	...
	if(xChkVatableStatus!=1){ //added here pra masave dn ung vat code ng item...
		//get rate
		var xz = $("#hdntaxcodes").val();
		taxnrate = 0;
		$.each(jQuery.parseJSON(xz), function() { 
		...
		uomoptions = uomoptions + "<input type='hidden' value='"+$("#hdncvat").val()+"' name=\"selitmvatyp\" id=\"selitmvatyp"+lastRow+"\" data-id='"+taxnrate+"'>";
	}
	...
	var tditmbaseamount = "<td width=\"100\" nowrap> <input type='text' value='"+curramtz+"' class='numeric form-control input-xs' style='text-align:right' name=\"txtntranamount\" id='txtntranamount"+lastRow+"' readonly> <input type='hidden' value='"+baseprice+"' name=\"txtnamount\" id='txtnamount"+lastRow+"'> </td>";
	...
	var tdsysno = "<td nowrap><input type='text' value='"+itmacctid+"' class='form-control input-xs' name=\"txtsytemno\" id='txtsytemno"+lastRow+"'> </td>";
								...
								$.each(data,function(index,item){
									if(item.citemno==""){
										...
										if (item.nqty>=1){
											...
											if(typ=="DR"){
												$("<tr>").append(
													$("<td align='center'>").html("<input type='checkbox' value='"+item.id+"' name='chkSales[]' data-id=\""+drno+"\" data-curr=\""+item.ccurrencycode+"\">"),
													$("<td>").text(item.creference),
													$("<td>").text(item.citemno),
													$("<td>").text(item.cdesc),
													$("<td>").text(item.cunit),
													$("<td>").text(item.nqty),
													$("<td align='center'>").text(item.nprice),
													$("<td align='center'>").text(item.nbaseamount),
													$("<td>").text(item.ccurrencycode)
												).appendTo("#MyInvDetList tbody");

												$xctblnme = "MyInvDetList";
												...
												$("<tr>").append(
													...
													$("<td>").text(item.nqty),
													$("<td align='center'>").text(item.nprice),
													$("<td align='center'>").text(item.namount)
												).appendTo("#MyQODetList tbody");

												$xctblnme = "MyQODetList";
											}
	...
	$.ajax ({
		...
		success: function( data ) {
			...
			$.each(data,function(index,item){
				...
				$("#hdnacctno").val(""); 
				$("#hdnacctid").val(item.citmsysno); 
				$("#hdnacctdesc").val(item.citmposno); 
				...
				//if(index>0){
					...
					var nfactor = $(this).find('input[type="hidden"][name="hdnfactor"]').val();
						...
						$.ajax ({
							...
							data: { trancode: trancode, crefno: crefno, crefident:crefident, indx:$xinx, citmno: citmno, cuom: cuom, nqty:nqty, nprice: nprice, ndiscount:ndiscount, ntranamt:ntranamt, namt:namt, mainunit:mainunit, nfactor:nfactor, ccode:ccode, vatcode:vatcode, nrate:nrate, ewtcode:ewtcode, ewtrate:ewtrate, itmsysno: itmsysno, itmposno: itmposno },
							async: false,
							success: function( data ) {
