...
<body style="padding:5px">
	<form action="RFP_void_tran.php" name="frmunpost" id="frmunpost" method="POST">
		...
		<div>
			<section>
              ...
              <th>AP Gross</th>
					...
					$result=mysqli_query($con,"select a.*, b.cname, c.cref from rfp a left join suppliers b on a.compcode=b.compcode and a.ccode=b.ccode left join (Select ctranno, GROUP_CONCAT(capvno) as cref from rfp_t where compcode='".$_SESSION['companyid']."' Group By ctranno) c on a.ctranno=c.ctranno where a.compcode='$company' and a.ctranno in ('".implode("','",$refrfp)."') and (a.lapproved=1 and a.lvoid=0) order by a.ddate desc");
					...
					?>
						<tr>
							...
							<td><?php echo str_replace(",","<br>",$row['cref']);?></td>
							<td><?php echo $row['ccode'];?> - <?php echo $row['cname'];?> </td>
              <td align="right"><?php echo  date_format(date_create($row['dtransdate']), "m/d/Y");?></td>
