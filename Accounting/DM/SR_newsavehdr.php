...
$chkSales = mysqli_query($con,"select * from aradj where compcode='$company' and ctype='Debit' and YEAR(ddate) = YEAR(CURDATE()) Order By ctranno desc LIMIT 1");
if (mysqli_num_rows($chkSales)==0) {
	$cSINo = "DM".$dmonth.$dyear."00000";
}
else {
	...
	if (!mysqli_query($con, "INSERT INTO aradj(`compcode`, `ctranno`, `ccode`, `cremarks`, `ddate`, `dcutdate`, `ngross`, `ctype`, `cpreparedby`, `cacctcode`, `cvatcode`) values('$company', '$cSINo', '$cCustID', $cRemarks, NOW(), STR_TO_DATE('$dDelDate', '%m/%d/%Y'), '$nGross', '$ctype', '$preparedby', $cacctcode, $cvatcode)")) {
		...
		values('$company','$cSINo','$preparedby',NOW(),'INSERTED','DEBIT MEMO','$compname','Inserted New Record')");
