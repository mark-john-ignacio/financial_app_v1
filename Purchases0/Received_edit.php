...
$sqlhead = mysqli_query($con,"select a.*,b.cname from receive a left join suppliers b on a.compcode=b.compcode and a.ccode=b.ccode where a.compcode='$company' and a.ctranno = '$ctranno'");
...
if (mysqli_num_rows($sqlhead)!=0) {
	while($row = mysqli_fetch_array($sqlhead, MYSQLI_ASSOC)){
		...
		$Date = $row['dcutdate'];
		$RecDate = $row['dreceived'];
		$SalesType = $row['creceivetype'];
		$Gross = $row['ngross'];
...
<form action="Received_editsave.php" name="frmpos" id="frmpos" method="post" onSubmit="return false;">
	<fieldset>
    ...
    <td><input type="text" id="txtnGross" name="txtnGross" readonly value="<?php echo $Gross;?>" style="text-align:right; border:none; background-color:#FFF; font-size:20px; font-weight:bold; color:#F00;" size="10"></td>
         ...
         <div class="alt2" dir="ltr" style="
            ...
            <table id="MyTable" class="MyTable" cellpadding"3px" width="100%" border="0">
					...
					<tbody class="tbody">
                    <?php 
						$sqlbody = mysqli_query($con,"select a.*,b.citemdesc,b.cunit as mainunit from receive_t a left join items b on a.citemno=b.cpartno and a.compcode=b.compcode where a.compcode='$company' and a.ctranno = '$ctranno' order by nident");
                    ...
                    <tr>
                        ...
                        <td style="width:100px; padding:1px"><input type='text' value='<?php echo $rowbody['nprice'];?>' class='numeric form-control input-xs' style='text-align:right' name='txtnprice<?php echo $cntr;?>' id='txtnprice<?php echo $cntr;?>' ></td>
                        <td style="width:100px; padding:1px"><input type='text' value='<?php echo $rowbody['namount'];?>' class='form-control input-xs' style='text-align:right' name='txtnamount<?php echo $cntr;?>' id='txtnamount<?php echo $cntr;?>' readonly></td>
                         <td style="width:100px; padding:1px; padding-left:10px">
                         <input type='text' value='<?php echo $rowbody['nfactor'];?>' name='txtnfactor<?php echo $cntr;?>' id='txtnfactor<?php echo $cntr;?>' class='nqty form-control input-xs' style='text-align:right'>
                         </td>
...
<table width="100%" border="0" cellpadding="3">
  ...
  <button type="button" class="btn btn-primary btn-sm" tabindex="6" onClick="window.location.href='Received.php';" id="btnMain" name="btnMain">
Back to Main<br>(ESC)</button>
   
    <button type="button" class="btn btn-default btn-sm" tabindex="6" onClick="window.location.href='Received_new.php';" id="btnNew" name="btnNew">
New<br>(F1)</button>
...
?>
<form action="Received_edit.php" name="frmpos2" id="frmpos2" method="post">
  <fieldset>
  <legend>Receiving</legend>	
<table width="100%" border="0">
  <tr>
  ...
  <tr>
    <tH colspan="4" align="center" style="padding:10px"><font color="#FF0000"><b>RR No. DID NOT EXIST!</b></font></tH>
    </tr>
</table>
</fieldset>
</form>
	...
	$(document).keypress(function(e) {	 
	 if(e.keyCode == 112) { //F1
		if(document.getElementById("btnNew").className=="btn btn-default btn-sm"){
			window.location.href='Received_new.php';
		}
	  }
...
function chkSIEnter(keyCode,frm){
	if(keyCode==13){
		document.getElementById(frm).action = "Received_edit.php";
...
function printchk(x){
	if(document.getElementById("hdncancel").value==1){	
		...
		document.getElementById("statmsgz").style.color = "#FF0000";
		  ...
		  var url =  "Received_confirmprint.php?x="+x;
