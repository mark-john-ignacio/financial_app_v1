...
//DEBITACCOUNT
	$sqlchk = mysqli_query($con,"Select * From parameters where compcode='$company' and ccode='DEPDEBIT'");
	if (mysqli_num_rows($sqlchk)!=0) {
		while($row = mysqli_fetch_array($sqlchk, MYSQLI_ASSOC)){
			$nDebitDef = $row['cvalue'];
		}
		
			if ($nDebitDef<>$nDebit){ UPDATEVAL('DEPDEBIT',$nDebit); }

	}
	else{ 
		if($nDebit<>"") { INSERTVAL('DEPDEBIT',$nDebit); }
	}
...
//INSERT LOGFILE
	...
	echo "Deposit Setup Succesfully Saved! ".$nDebit;
?>
