...
include('../../include/denied.php');
	...
	$csalesno = $_REQUEST['x'];
	$sqlhead = mysqli_query($con,"select a.*,b.cname from quote a left join customers b on a.compcode=b.compcode and a.ccode=b.cempid where a.compcode='$company' and a.ctranno = '$csalesno'");
...
function Print(x){
				...
				location.href = "Quote_print.php?x="+x;
}
...
<body>
<br><br>
<table border="1" cellpadding="3" style="width: 8in; height: 10.5in" id="tblMain">
  <tr>
    <td><font size="3"><b><?php echo $companyname;?></b></font></td>
    <td colspan="2" align="center"><font size="3"><b>Quotation</b></font></td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <table width="100%" border="0" cellpadding="3" style="border-style:dashed;">
      ...
      <?php 
		$sqlbody = mysqli_query($con,"select a.*,b.citemdesc from quote_t a left join items b on a.compcode=b.compcode and a.citemno=b.cpartno where a.compcode='$company' and a.ctranno = '$csalesno'");
	...
	?>
      ...
      <tr>
        ...
        <td style="border-right:1px dashed;" align="right"><?php echo $cCurrCode." ".$rowbody['nprice'];?></td>
...
<div style="float:left;">&nbsp;&nbsp;<strong><font size="-1">QUOTATION</font></strong></div>
...
if($lPosted==0 && $autopost==1){
	...
	$valsub = "PRINT AND POST QUOTATION";
}
else{
	$strqry = "PrintRed('".$csalesno."')";
	$valsub = "PRINT QUOTATION";
}
