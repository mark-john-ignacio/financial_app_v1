<?php
	...
	//get all so
	$resq = mysqli_query ($con, "Select A.*, B.cvattype, C.cpartno, C.cskucode, C.citemdesc From so_t A left join quote B on A.compcode=B.compcode and A.ctranno=B.ctranno left join items C on A.compcode=C.compcode and A.citemno=C.cpartno  where A.compcode='$company' and A.ctranno = '".$_REQUEST['id']."' and A.nident = '".$_REQUEST['itm']."'");
