...
$spreadsheet->getProperties()->setCreator('Myx Financials')
    ...
    ->setDescription('Customers, generated using Myx Financials.')
    ->setKeywords('myx_financials customers')
    ->setCategory('Myx Financials Customers');
...
$company = $_SESSION['companyid'];
	...
	$xsql = "Select * From customers where compcode='$company'";
	...
	foreach($finarray as $row)
	{
		...
		$spreadsheet->setActiveSheetIndex(0)
    ->setCellValue('A'.$cnt, $row['cempid'])
    ->setCellValue('B'.$cnt, $row['cname'])
		->setCellValue('C'.$cnt, $row['ctradename'])
		->setCellValue('D'.$cnt, $row['ctin'])
		->setCellValue('E'.$cnt, $row['ccustomertype'])
		->setCellValue('F'.$cnt, $row['ccustomerclass'])
...
header('Content-Disposition: attachment;filename="CustomersList.xlsx"');
