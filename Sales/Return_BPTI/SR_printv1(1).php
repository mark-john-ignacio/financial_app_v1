<?php
    ...
    $sql = "SELECT a.*, b.*, c.Fname, c.Lname FROM salesreturn a
        left join customers b on a.compcode = b.compcode and  a.ccodee = c.cempid
        left join users c on a.cpreparedby = c.Userid
        where a.compcode = '$company' and a.ctranno = '$tranno'";
    $query = mysqli_query($con, $sql);
    while($row = $query -> fetch_assoc()){
        $CustName = $row['cname'];
        $CustAdd = $row['chouseno'] . " " . $row['ccity'] . " " . $row['cstate'] . " " . $row['country'] ;
	...
	<tr>
		<td style="vertical-align: top; padding-top: 10px">
			...
			<table border="0" align="center" width="100%" style="border-collapse: collapse;">
				...
				<?php 
                $sql = "SELECT a.*, b.cpartno, b.citemdesc, b.cunit FROM salesreturn_t a
                    left join items b on a.compcode = b.comcode and a.cpartno = b.citemno
                    where a.compcode = '$company' and a.ctranno = '$tranno'";
