...
$_SESSION['pageid'] = "SuppInv_unpost.php";
	...
	<form action="RR_void_tran.php" name="frmunpost" id="frmunpost" method="POST">
		...
		<div>
			<section>
				...
				<button type="button" class="btn btn-danger btn-sm" id="btnsubmit" name="btnsubmit"><span class="fa fa-times"></span>&nbsp;Un-Post Transaction</button>
				...
				<table id="example" class="table table-hover " cellspacing="1" width="100%">
					...
					<tbody>
					<?php
					$alrr = mysqli_query($con,"Select a.crefno from apv_d a left join apv b on a.compcode=b.compcode and a.ctranno=b.ctranno where a.compcode='$company' and b.lcancelled=0 and b.lvoid=0");
					$refpos[] = "";
					while($rowxcv=mysqli_fetch_array($alrr, MYSQLI_ASSOC)){
						$refpos[] = $rowxcv['crefno'];
					}

					$result=mysqli_query($con,"select a.*,b.cname from suppinv a left join suppliers b on a.compcode=b.compcode and a.ccode=b.ccode where a.compcode='$company' and a.ctranno not in ('".implode("','",$refpos)."') and (a.lapproved=1 and a.lvoid=0) order by a.ddate desc");
					...
					?>
						<tr>
							...
							<td><?php echo $row['crefsi'];?></td>
							<td><?php echo $row['crefrr'];?></td>
