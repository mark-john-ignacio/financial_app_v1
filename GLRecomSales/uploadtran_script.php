...
function getSetAcct($id){
	...
	if($typ=="PRet"){
		...
		//Accounts Payable -> supplier account -> Debit
			if (!mysqli_query($con,"INSERT INTO `glactivity`(`compcode`, `cmodule`, `ctranno`, `ddate`, `acctno`, `ctitle`, `ndebit`, `ncredit`, `lposted`, `dpostdate`) Select '$company', 'PV', '$tran', A.dcvdate, A.ccustacctcode, B.cacctdesc, A.ngross, 0, 0, NOW() From paybill A left join accounts B on A.compcode=B.compcode and A.ccustacctcode=B.cacctno where A.compcode='$company' and A.ctranno='$tran' ")){
				echo "False";
			}
			else{
