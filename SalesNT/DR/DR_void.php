...
<body style="padding:5px">
	<form action="DR_void_tran.php" name="frmunpost" id="frmunpost" method="POST">
		...
		<div>
			<section>
				...
				<table id="example" class="table table-hover " cellspacing="1" width="100%">
					...
					<tbody>
					<?php
					$alrr = mysqli_query($con,"Select a.creference from ntsales_t a left join ntsales b on a.compcode=b.compcode and a.ctranno=b.ctranno where a.compcode='$company' and b.lcancelled=0 and b.lvoid=0");
					$refpos[] = "";
					while($rowxcv=mysqli_fetch_array($alrr, MYSQLI_ASSOC)){
						$refpos[] = $rowxcv['creference'];
					}
					
					$result=mysqli_query($con,"select a.*,IFNULL(b.ctradename,b.cname) as cname, D.cref from ntdr a left join customers b on a.compcode=b.compcode and a.ccode=b.cempid LEFT JOIN (Select x.ctranno, GROUP_CONCAT(DISTINCT x.creference) as cref from `ntdr_t` x where x.compcode='".$_SESSION['companyid']."' group by x.ctranno) D on a.ctranno=D.ctranno where a.compcode='$company' and a.ctranno not in ('".implode("','",$refpos)."') and (a.lapproved=1 and a.lvoid=0) order by a.ddate desc");
