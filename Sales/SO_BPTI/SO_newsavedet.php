...
$company = $_SESSION['companyid'];
		...
		$citmrmpo = chkgrp($_REQUEST["citmrempo"]); 
		$citmrmdneed = $_REQUEST["citmremdneed"]; 
	...
	if (!mysqli_query($con,"INSERT INTO so_t(`compcode`, `cidentity`, `ctranno`, `creference`, `nrefident`, `nident`, `citemno`, `nqty`, `cunit`, `nprice`, `namount`, `nbaseamount`, `cmainunit`,`nfactor`,`ctaxcode`, `nrate`, `ditempono`, `ditemneeded`, `citemremarks`) values('$company', '$refcidenttran', '$cSINo', $crefno, '$nrefident', '$indexz', '$cItemNo', '$nQty', '$cUnit', '$nPrice', '$nAmount', '$nBaseAmount', '$cMainUOM', $nFactor, '$ctaxcode', '$nrate', $citmrmpo, STR_TO_DATE('$citmrmdneed', '%m/%d/%Y') ,$citmrmx)")){
		echo "False";
	}
	else{
		echo "True";
