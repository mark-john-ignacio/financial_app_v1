...
$sqlhead = mysqli_query($con,"select a.ctranno, a.crefno, a.lcancelled, a.lapproved, a.cremarks, DATE_FORMAT(a.ddate,'%m/%d/%Y') as ddate, DATE_FORMAT(a.dreceived,'%m/%d/%Y') as dreceived from receive_putaway a where a.compcode='$company' and a.ctranno = '$cpono'");
...
?>
	...
	<fieldset>
    	<legend>Receiving/Putaway</legend>	
...
<tr>
 ...
 <tr>
  ...
  <tr>
    ...
    <tH width="150" style="padding:2px">INVENTORY DATE:</tH>
    <td style="padding:2px">
    <div class="col-xs-8">
...
Back to Main<br>(ESC)</button>
    ...
    <button type="button" class="btn btn-default btn-sm" tabindex="6" onClick="window.location.href='InvRec_new.php';" id="btnNew" name="btnNew">
New<br>(F1)</button>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body" style="height:20vh">
								...
								<div class="row">
										...
										<div class="col-xs-2 nopadwtop"><input type="text" class="form-control input-sm" name="serdisuom" id="serdisuom" readonly="true"></div>
...
$(function(){	
	...
	$("#btnInsSer").on("click", function(){
			...
			InsertToSerials(itmcode,serials,uoms,qtys,locas,locasdesc,expz,itmcoderefident);
...
function InsertToSerials(itmcode,serials,uoms,qtys,locas,locasdesc,expz,nident){

	$("<tr>").append(
		$("<td width=\"120px\" style=\"padding:1px\">").html("<input type='hidden' value='"+itmcode+"' name=\"sertabitmcode\" id=\"sertabitmcode\"><input type='hidden' value='"+nident+"' name=\"sertabident\" id=\"sertabident\">"+itmcode),
...
function InsertDetSerial(itmcode, itmname, itmunit, itemrrident){
...
function InsertSerialsList(){	
             ...
             $.each(data,function(index,item){
								...
								InsertToSerials(item.citemno,item.cserial,item.cunit,item.nqty,item.nlocation,item.locadesc,item.dexpired,item.nrefidentity);
...
function chkform(){
	...
	if(document.getElementById("txtrrref").value==""){
			...
			$("#AlertMsg").html("&nbsp;&nbsp;Reference RR Required!");
			$("#alertbtnOK").show();
	...
	var isDone = "True";
		...
		//Saving the header
		var ctranno = $("#txtcpono").val();
		var rrno = $("#txtrrref").val();
		var crem = $("#txtremarks").val();
		var ddaterec = $("#date_received").val();
				
		$.ajax ({
			url: "InvRec_editsave.php",
			data: { ctranno:ctranno, rr: rrno, crem: crem, ddate: ddaterec },
			async: false,
			beforeSend: function(){
				$("#AlertMsg").html("&nbsp;&nbsp;<b>UPDATING TRANSACTION: </b> Please wait a moment...");
				$("#alertbtnOK").hide();
				$("#AlertModal").modal('show');
			},
			success: function( data ) {
