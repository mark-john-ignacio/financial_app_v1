		...
		if($chkwarn==1){ //Check if blocking is enabled... pag enabled chk limit b4 posting
			...
			if (mysqli_num_rows($sqlhead)!=0) {
				...
				if((float)$chkbal >= (float)$grossxz){
						...
						if (!mysqli_query($con,"Update ntdr set lapproved=1 where compcode='$company' and ctranno='$tranno'")) {
...
if($_REQUEST['typ']=="CANCEL"){
	...
	if (!mysqli_query($con,"Update ntdr set lcancelled=1 where compcode='$company' and ctranno='$tranno'")) {
