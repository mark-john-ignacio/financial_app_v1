...
class Customers extends BaseController
{
    ...
    {
        return view($this->view.'upload_index', ['errors' => []]);
    }
    ...
    public function upload()
    {
        ...
        if ($this->request->getMethod() == 'post') {
            $filesize = $this->db->table('system_settings')->select('value')->where('ccode', 'excel')->where('compcode', $this->company_code)->where('deleted', 0)->get()->getRow();
            if (!$filesize) {
            ...
            $input = $this->validate([
                'file' => [
                    ...
                    'max_size[file,' . $filesizeValue . ']',
                ],
            ]);
    
            if (!$input) {
                $data['validation'] = $this->validator;
                $this->swal('e', 'Invalid File');
                return redirect()->to(base_url() . '/admin/custmasterlist');
            } else {
                $file = $this->request->getFile('file');
                $spreadsheet = IOFactory::load($file->getTempName());

                $template = ROOTPATH . 'public/downloadable_templates/customertemplate.xlsx';
                ...
                if (!$isValid) {
                    $this->swal('e', 'Invalid File! Please download the template');
                    return redirect()->to(base_url() . '/admin/custmasterlist');
                }
                ...
                @$isload = $file->move(ROOTPATH . 'public/uploads/mass_upload/customers/', $newName);
                ...
                if ($sheet1Data) {
                    ...
                    foreach ($sheet1Data as $index => $row) {
                        ...
                        if (!empty($rowData[$custcode])){
                            ...
                            $this->swal('e', 'Empty Customer Code!');
                             return redirect()->to(base_url() . '/admin/custmasterlist');
                        }
            ...
            return view('admin/' . session()->get('default_menu'), $data);
        }
    }
    ...
    public function inserttblCustomers()
    {
    ...
    {  
        ...
        foreach ($data as $row) {
            ...
            $rowData['created_date'] = datetimedb; 
            ...
            if (!empty($rowData['cempid']) && !empty($rowData['cname'])) {
                ...
                $saveSuccess = $this->db->table('customers')->insert($rowData);
