	...
	<div>
        ...
        <div>
        	<div style="float:left; width:50%">
				<font size="+2"><u>Customers Master List > Classification</u></font>	
            </div>
			...
			<table id="example" class="display" cellspacing="0" width="100%">
				...
				$company = $_SESSION['companyid'];
					...
					$sql = "select * from groupings where compcode='$company' and ctype='CUSTCLS' order by cdesc";
  ...
  <div class="modal-dialog modal-md">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="myModalLabel"><b>Add New Classification</b></h5>        
      </div>
...
?>
	...
	$(function(){
		...
		$("#btnadd").on("click", function() {
		 var x = chkAccess('CUSCLASS_New.php');
		 ...
		 if(x.trim()=="True"){
			...
			$('#myModalLabel').html("<b>Add New Classificationl</b>");
			$('#myModal').modal('show');
		 } else {
	...
	function editgrp(code,desc){
		 var x = chkAccess('CUSCLASS_Edit.php');
		 ...
		 if(x.trim()=="True"){
			...
			$('#myModalLabel').html("<b>Update Classification Detail</b>");
			$('#myModal').modal('show');
		 } else {
			 $("#AlertMsg").html("<center><b>ACCESS DENIED!</b></center>");
			 $("#AlertModal").modal('show');
		...
		function setStat(code, stat){
			$.ajax ({
				...
				data: { code: code,  stat: stat, typz: 'CUSTCLS' },
				async: false,
				success: function( data ) {
					if(data.trim()!="True"){
