<?php
	...
	$posnew = "True";
	$sql = mysqli_query($con,"select * from users_access where userid = '$employeeid' and pageid = 'MESLocations_new'");
	if(mysqli_num_rows($sql) == 0){
		$posnew = "False";
	}

	$posedit = "True";
	$sql = mysqli_query($con,"select * from users_access where userid = '$employeeid' and pageid = 'MESLocations_edit'");
	if(mysqli_num_rows($sql) == 0){
		$posedit = "False";
	}
	...
	<div>
      ...
      <div>
        <div style="float:left; width:50%">
					<font size="+2"><u>Locations List	</u></font>	
        </div>            
      </div>
			...
			<table id="example" class="display" cellspacing="0" width="100%">
				...
				<tbody>
					<?php
						...
						$sql = "select * from mrp_locations where compcode='$company'order by cdesc";							
		...
		<div class="modal-dialog modal-md">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="myModalLabel"><b>Add New Location </b></h5>        
				</div>
				...
				<div class="modal-body" style="height: 20vh">
						...
						<div class="col-xs-12">
								...
								<div class="col-xs-9 nopadwtop">
									<input type="text" class="form-control input-sm text-uppercase" id="txtcdesc" name="txtcdesc"  placeholder="Enter Name.." required>
	...
	<script>
		...
		$(function(){
			...
			$("#btnadd").on("click", function() {
				...
				if(x.trim()=="True"){
					...
					$('#myModalLabel').html("<b>Add New Location</b>");
					$('#myModal').modal('show');
				} else {
			...
			$("#btnSave, #btnUpdate").on("click", function() {
				...
				if(vardesc==""){
					...
					if(xcstat=="False"){
						...
						$("#add_err").html("<center><b>Description Already Exist!</b></center>");
						$("#add_err").show();
					}
		...
		function editgrp(code,desc){
			...
			if(x.trim()=="True"){
				...
				$('#myModalLabel').html("<b>Update Location's Description</b>");
				$('#myModal').modal('show');
			} else {
				$("#AlertMsg").html("<center><b>ACCESS DENIED!</b></center>");
				$("#AlertModal").modal('show');
