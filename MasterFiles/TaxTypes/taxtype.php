<?php
	...
	$posnew = "True";
	$sql = mysqli_query($con,"select * from users_access where userid = '$employeeid' and pageid = 'TaxType_New'");
	if(mysqli_num_rows($sql) == 0){
		$posnew = "False";
	}

	$posedit = "True";
	$sql = mysqli_query($con,"select * from users_access where userid = '$employeeid' and pageid = 'TaxType_Edit'");
	if(mysqli_num_rows($sql) == 0){
		$posedit = "False";
	}
	...
	<div>
      ...
      <div>
        <div style="float:left; width:50%">
					<font size="+2"><u>Tax Types List</u></font>	
        </div>            
      </div>
			...
			<table id="example" class="display" cellspacing="0" width="100%">
				...
				<tbody>
					<?php
						...
						$sql = "select * from vatcode where compcode='$company' order by cvatdesc";							
						...
						{
							$arrdesc[] = $row['cvatcode'];
					?>
						<tr>
							<td>
								<a href="javascript:;" onClick="editgrp('<?php echo $row['nidentity'];?>','<?php echo $row['cvatcode'];?>','<?php echo $row['cvatdesc'];?>','<?php echo $row['cremarks'];?>','<?php echo $row['nrate'];?>','<?php echo $row['ctype'];?>')">
									<?php echo $row['cvatcode'];?>
								</a>
								
							</td>
							<td> <?php echo $row['cvatdesc'];?> </td>
							<td> <?php echo $row['cremarks'];?><div class="itmalert alert alert-danger nopadding" id="itm<?php echo $row['nidentity'];?>" style="display: inline";></div></td>
		...
		<div class="modal-dialog modal-md">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="myModalLabel"><b>Add New Tax Type</b></h5>        
				</div>
				...
				<div class="modal-body" style="height: 30vh">
						...
						<div class="col-xs-12">
							...
							<div class="col-xs-9 nopadwtop">
								<input type="text" class="form-control input-sm" id="txttaxrems" name="txttaxrems"  placeholder="Remarks.." required>
							</div>
						</div>
						<div class="col-xs-12">
							<div class="cgroup col-xs-3 nopadwtop">
						...
						<div class="col-xs-12">
							...
							<div class="col-xs-3 nopadwtop">
								<select class="form-control input-sm" id="seltaxtype" name="seltaxtype" required>
...
?>
  ...
  <link rel="stylesheet" type="text/css" href="../../Bootstrap/DataTable/DataTable.css"> 
	...
	<script>
		...
		$(function(){
			...
			$("#btnadd").on("click", function() {
				...
				if(x.trim()=="True"){
					...
					$('#myModalLabel').html("<b>Add New Tax Type</b>");
					$('#myModal').modal('show');
				} else {
			...
			$("#btnSave, #btnUpdate").on("click", function() {
				...
				var taxrems = $("#txttaxrems").val();
				var taxrate = $("#txttaxrate").val();
				var taxtype = $("#seltaxtype").val();
				...
				if(taxcode=="" || taxdesc==""){
					...
					if(xcstat=="False"){
						$.ajax ({
							url: "th_save.php",
							data: { id:varcode, taxcode: taxcode, taxdesc: taxdesc, taxrems: taxrems, taxrate: taxrate, taxtype: taxtype },
							async: false,
						...
						$("#add_err").html("<center><b>Tax Code Already Exist!</b></center>");
						$("#add_err").show();
					}
		...
		function editgrp(id,code,desc,rems,rate,type){
			var x = "<?=$posedit;?>";
			...
			if(x.trim()=="True"){
				...
				$('#myModalLabel').html("<b>Update Tax Type Details</b>");
				$('#myModal').modal('show');
			} else {
				$("#AlertMsg").html("<center><b>ACCESS DENIED!</b></center>");
				$("#AlertModal").modal('show');
