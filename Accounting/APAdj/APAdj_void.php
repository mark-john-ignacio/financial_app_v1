<?php
	...
	$_SESSION['pageid'] = "APAdj_unpost.php";
...
<body style="padding:5px">
	<form action="APAdj_void_tran.php" name="frmunpost" id="frmunpost" method="POST">
	
		<div>
			<section>
					<div>
						<div style="float:left; width:50%">
							<font size="+2"><u>AP Adjustment List</u></font>	
						</div>
					</div>
				<br><br>
              ...
              <th>Remarks</th>
					...
					<tbody>
					<?php
					$alrr = mysqli_query($con,"Select a.crefapcm from apv_deds a left join apv b on a.compcode=b.compcode and a.ctranno=b.ctranno where a.compcode='$company' and b.lcancelled=0 and b.lvoid=0");
					$refpos[] = "";
					while($rowxcv=mysqli_fetch_array($alrr, MYSQLI_ASSOC)){
						$refpos[] = $rowxcv['crefapcm'];
					}

					$result=mysqli_query($con,"select A.*, B.cname from apadjustment A left join suppliers B on A.compcode=B.compcode and A.ccode=B.ccode where A.compcode='$company' and a.ctranno not in ('".implode("','",$refpos)."') and (a.lapproved=1 and a.lvoid=0)  order by A.ddate DESC");
