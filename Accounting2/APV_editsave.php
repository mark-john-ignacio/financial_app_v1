...
$company = $_SESSION['companyid'];
	...
	if (!mysqli_query($con, "UPDATE `apv` set `dapvdate` = STR_TO_DATE('$dTranDate', '%m/%d/%Y'), `ccode` = '$cCustID', `cpayee` = '$cPayee', `cpaymentfor` = '$cRemarks', `ngross` = '$nGross' Where `compcode` = '$company' and `ctranno` = '$cPVNo'")) {
		printf("Errormessage: %s\n", mysqli_error($con));
		echo "<br> UPDATE `apv` set `dapvdate` = STR_TO_DATE('$dTranDate', '%m/%d/%Y'), `ccode` = '$cCustID, `cpayee` = '$cPayee',`cchkno` = '$cChkNo', `cpaymentfor` = '$cRemarks' Where compcode` = '$company' and `ctranno` = '$cPVNo'";
	} 
	...
	for($z=1; $z<=$rowcnt; $z++){
		...
		mysqli_query($con,"INSERT INTO `apv_d`(`compcode`, `nidentity`, `ctranno`, `crefno`, `crefinv`, `cdescription`, `namount`, `cremarks`) values('$company', '$z', '$cPVNo', '$crrno', '$suppsi', '$desc', $amnt, '$remarks')");
	...
	for($z=1; $z<=$rowcnt; $z++){
		...
		mysqli_query($con,"INSERT INTO `apv_t`(`compcode`, `nidentity`, `ctranno`, `crefrr`, `cacctno`, `ctitle`, `cremarks`, `csubsidiary`, `ndebit`, `ncredit`)values('$company', '$z', '$cPVNo', '$crefrr', '$cacctno', '$ctitle', '$cacctrem', '$nsubid', $ndebit, $ncredit)");
