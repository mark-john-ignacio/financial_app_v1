...
function myFunctionadd(prodid,prdnme,cunit,nqty,nfactor,cmainunit,xref,nident){
	...
	z.innerHTML = "<input class='btn btn-info btn-xs' type='button' id='ins" + prodid + "' value='insert' />";			
									...
									$("#ins"+prodid).on('click', function() {
										 InsertDetSerial(prodid,prdnme,cmainunit,nident,nqty,nfactor,cunit);
									});					
									
}

function InsertDetSerial(itmcode, itmname, itmunit, itemrrident, itemqty, itmfctr, itemcunit){
...
//alert("th_serialslist-manual.php?itm="+itmcode+"&cuom="+itmunit+"&qty="+itemqty+"&factr="+itmfctr+"&mainuom="+itemcunit);
	...
	$('#MyTableSerials tbody').empty();
			...
			$.ajax({
					...
					data: { itm: itmcode, cuom: itmunit, qty: itemqty, factr: itmfctr, mainuom: itemcunit },
					type: "POST",
					async: false,
					dataType: "JSON",
					success: function(data)
	...
	var isDone = "True";
		...
		if(trancode!=""){
			...
			$("#MyTable2 > tbody > tr").each(function(index) {	
				...
				var crefidnt = $(this).find('input[type="hidden"][name="sertabreferid"]').val();
