...
<form action="OR_newsave2.php" name="frmOR" id="frmOR" method="post">
  ...
  <tr>
    ...
    <?php
    $sqlchk = mysqli_query($con,"Select a.cvalue, b.cacctdesc, IFNULL(b.nbalance,0) as nbalance From parameters a left join accounts b on a.compcode=b.compcode and a.cvalue=b.cacctno where a.compcode='$company' and a.ccode='ORDEBIT'");
    ...
    <div class="col-xs-8">
     ...
     <select id="selpaytype" name="selpaytype" class="form-control input-sm selectpicker">
      ...
      <!--
       ...
       <option value="StPeter">St. Peter Loan</option>
  ...
  <tr>
	...
	<?php
	$ornostat = "";
    	$sqlchk = mysqli_query($con,"select * from receipt where compcode='$company' Order By cornumber desc LIMIT 1");
	if (mysqli_num_rows($sqlchk)!=0) {
		while($row = mysqli_fetch_array($sqlchk, MYSQLI_ASSOC)){
