...
function chkgrp($valz) {
	...
	if (!mysqli_query($con,"INSERT INTO ntsalesreturn_t(`compcode`, `cidentity`, `ctranno`, `creference`, `nident`, `nrefident`,`citemno`, `nqty`, `nqtyorig`, `cunit`, `cmainunit`,`nfactor`,`creason`,`nprice`,`nbaseamount`,`namount`,`ccurrcode`,`ncurrate`) values('$company', '$refcidenttran', '$cSINo', $crefno, '$indexz', '$nrefident','$cItemNo', '$nQty', '$nOrigQty', '$cUnit',  '$cMainUOM', $nFactor, $creason, '$nPrice', '$nAmount', '$nTranAmount', '$CurrCode', '$CurrRate')")){
		...
		$ngross = 0;
		$nbasegross = 0;
		$chkSales = mysqli_query($con,"select * from ntsalesreturn_t where compcode='$company' and ctranno='$cSINo'");
		...
		$chkSales = mysqli_query($con,"update ntsalesreturn set ngross='$ngross', nbasegross='$nbasegross' where compcode='$company' and ctranno='$cSINo'");
