...
<form action="SI_edit.php" name="frmpos" id="frmpos" method="post">
	<fieldset>
    	<legend>
        <div class="col-xs-6 nopadding">SI Non-Trade Details
...
function myFunctionadd(qty,pricex,ndisc,curramt,amtx,factr,cref,nrefident,citmcls){
  ...
  var itsvats = "";
	...
	var tditmprice = "<td width=\"100\" nowrap> <input type='text' value='"+price+"' class='numericdec form-control input-xs' style='text-align:right' name=\"txtnprice\" id='txtnprice"+lastRow+"' > </td>";
	...
	$('#MyTable > tbody:last-child').append('<tr>'+tditmcode + tditmdesc + tditmunit + tditmqty + tditmprice + tditmdisc + tditmbaseamount + tditmamount + tditmdel + '</tr>');
		...
		function ComputeAmt(nme){
			...
			//alert(parseFloat(nprc) + "*" + (parseFloat(ndsc) / 100));
			
			if (parseFloat(ndsc) != 0) {
				ntdisc = parseFloat(nprc) * (parseFloat(ndsc) / 100);
				
				nprc = parseFloat(nprc) - parseFloat(ntdisc);
			}
