...
<form action="PurchRet_editsave.php" name="frmpos" id="frmpos" method="post" onSubmit="return false;">
  ...
  <tr>
    ...
    <td colspan="2" style="padding:2px"><div class="col-xs-4"><input type="text" class="form-control input-sm" id="txtctranno" name="txtctranno" width="20px" tabindex="1" value="<?php echo $ctranno;?>" onKeyUp="chkSIEnter(event.keyCode,'frmpos');"></div>
    
      <input type="hidden" name="hdntranno" id="hdntranno" value="<?php echo $ctranno;?>">
  ...
  <tr>
    ...
    <td style="padding:2px">
    	<div class="col-xs-7">
        	<input type="text" class="form-control input-sm" id="txtcust" name="txtcust" width="20px" tabindex="1" placeholder="Search Supplier Name..." value="<?php echo $CustName;?>" readonly autocomplete="off">
        </div> 
        &nbsp;&nbsp;
  ...
  <tr>
    ...
    <td style="padding:2px"><div class="col-xs-9"><input type="text" class="form-control input-sm" id="txtremarks" name="txtremarks"  tabindex="2" value="<?php echo $Remarks;?>"></div></td>
    <tH width="150" style="padding:2px">RETURN DATE:</tH>
    <td style="padding:2px">
    <div class="col-xs-8">
...
<tr>
    ...
    <td style="padding:2px">
    <input type="text" id="txtnGross" name="txtnGross" readonly value="<?php echo $Gross;?>" style="text-align:right; border:none; background-color:#FFF; font-size:20px; font-weight:bold; color:#F00;" size="15">
    </td>
         ...
         <div class="alt2" dir="ltr" style="
            ...
            <table id="MyTable" class="MyTable" cellpadding"3px" width="100%" border="0">
                      ...
                      <?php 
						...
						if (mysqli_num_rows($sqlbody)!=0) {
							...
							while($rowbody = mysqli_fetch_array($sqlbody, MYSQLI_ASSOC)){
								...
								if($rowbody['mainunit'] != $rowbody['cunit']){
									$cdesc = $rowbody['citemdesc']." (".$rowbody['nfactor']." ".$rowbody['mainunit']."/".$rowbody['cunit'].")";
								}
								else{
									$cdesc = $rowbody['citemdesc'];
								}
                    ...
                    <tr>
                        ...
                        <td><input type='hidden' value='<?php echo $rowbody['citemdesc'];?>' name='txtitemdesc<?php echo $cntr;?>' id='txtitemdesc<?php echo $cntr;?>'><?php echo $cdesc;?></td>
                        ...
                        <td style="width:100px; padding:1px"><input type='text' value='<?php echo ($rowbody['nqty']);?>' class='numeric form-control input-xs' style='text-align:right' name='txtnqty<?php echo $cntr;?>' id='txtnqty<?php echo $cntr;?>' > <input type='hidden' value='<?php echo ($rowbody['nqtyorig']);?>' name='txtnqtyOrig<?php echo $cntr;?>' id='txtnqtyOrig<?php echo $cntr;?>'></td>
                        ...
                        <td style="width:100px; padding:1px"><input type='text' value='<?php echo $rowbody['namount'];?>' class='form-control input-xs' style='text-align:right' name='txtnamount<?php echo $cntr;?>' id='txtnamount<?php echo $cntr;?>' readonly> <input type='hidden' value='<?php echo $rowbody['ncost'];?>' name='txtncost<?php echo $cntr;?>' id='txtncost<?php echo $cntr;?>'> <input type='hidden' value='<?php echo $rowbody['nretail'];?>' name='txtnretail<?php echo $cntr;?>' id='txtnretail<?php echo $cntr;?>'></td>
                         ...
                         <input type='text' value='<?php echo $rowbody['nfactor'];?>' name='txtnfactor<?php echo $cntr;?>' id='txtnfactor<?php echo $cntr;?>' class='numeric form-control input-xs' style='text-align:right'>
                         </td>
...
New<br>(F1)</button>
     ...
     <button type="button" class="btn btn-info btn-sm" tabindex="6" onClick="openinv();" id="btnqo" name="btnqo">RR<br> (Insert)</button>
...
<table width="100%" border="0">
  <tr>
    <tH width="100">PR NO.:</tH>
    <td colspan="3" style="padding:2px" align="left"><div class="col-xs-5"><input type="text" class="form-control input-sm" id="txtctranno" name="txtctranno" width="20px" tabindex="1" value="<?php echo $ctranno;?>" onKeyUp="chkSIEnter(event.keyCode,'frmpos2');"></div></td>
    </tr>
  <tr>
                ...
                <div class="form-group">
                    ...
                    <div class="col-xs-8 nopadwleft pre-scrollable">
                          <table name='MyInvDetList' id='MyInvDetList' class="table table-small">
                           <thead>
                            <tr>
                              <th align="center"> <input name="allbox2" id="allbox2" type="checkbox" value="Check All" /></th>
                              <th>Item No</th>
                              <th>Description</th>
                              <th>UOM</th>
                              <th>Qty</th>
...
$(document).ready(function() {
	...
	$('#txtsinum').typeahead({
		...
		displayText: function (item) {
			return '<div style="border-top:1px solid gray; width: 300px"><span >'+item.crrno+'</span><br><small><span class="dropdown-item-extra">' + item.dcutdate + '</span> ' + item.ngross + '</small></div>';
		},
		highlighter: Object,
		afterSelect: function(item) { 					
			 $("#txtsinum").val(item.crrno).change();
			 
			 if($("#txtcustid").val()==""){			
				$('#txtcust').val(item.cname); 
				$("#txtcustid").val(item.ccode);
				
				 $("#txtrefSI").val(item.crrno);
				 putSIdetail(item.crrno);
			 }else{
...
$('#MyInvTbl').on('click', "td[name='tditem']", function() {
	...
	$('#loadimg').show();
 	...
 	$("#allbox, #allbox2").click(function () {
        if ($("#allbox, #allbox2").is(':checked')) {
...
function putSIdetail(val){
	...
	$('#DRListHeader').html("RR List: " + val + "<div id='loadimg'><center><img src='../images/cusload.gif' style='show:none;'> </center> </div>");
	$('#MyDRDetList tbody').empty();
	$('#MyInvDetList tbody').empty();
...
function InsertSI(){	
   ...
   $("input[name='chkSales[]']:checked").each( function () {
				...
				var tranno = $("#txtrefSI").val();
	   			var id = $(this).val();
	   			$.ajax({
					url : "th_rrnumdetails.php?id=" + tranno + "&itm=" + id,
					type: "GET",
					dataType: "JSON",
					success: function(data)
					{	
...
function addItemName(){
	...
	var itmcost = document.getElementById("hdncost").value;
		...
		if(itmmunit!=itmmunit){
			itmdesc = itmdesc + "("+itmfactor+" "+itmmunit+"/"+itmunit+")";
		}
	...
	x.innerHTML = "<input type='text' value='"+itmprice+"' class='form-control input-xs' style='text-align:right' name='txtnprice"+lastRow+"' id='txtnprice"+lastRow+"' readonly >";
	y.innerHTML = "<input type='text' value='"+itmnamount+"' class='form-control input-xs' style='text-align:right' name='txtnamount"+lastRow+"' id='txtnamount"+lastRow+"' readonly> <input type='hidden' value='"+itmcost+"' name='txtncost"+lastRow+"' id='txtncost"+lastRow+"'> <input type='hidden' value='"+itmprice+"' name='txtnretail"+lastRow+"' id='txtnretail"+lastRow+"'>";
	w2.innerHTML = "<input type='text' value='"+itmfactor+"' name='txtnfactor"+lastRow+"' id='txtnfactor"+lastRow+"' class='numeric form-control input-xs' style='text-align:right'></div>";
                    ...
                    success: function (data) {
                       ...
                       $.each(data,function(index,item){
						...
						if(item.crrno=="NONE"){
							...
							$("<tr>").append(
							$("<td name='tditem'>").text(item.crrno),
							$("<td>").text(item.ngross)
