...
class Customers extends BaseController
{
    ...
    private function validateSheetData($sheetData)
    {
        ...
        foreach ($sheetData['sheet1'] as $index => $row) {
            ...
            // Combine headers with row data and trim values
            $rowData = array_combine($headers, array_map('trim', $row));
    
            // Debugging: Print row data
            error_log("Row $index: " . json_encode($rowData));
    ...
    private function validateRowData($rowData, $requiredFields)
    {
        ...
        if (!empty($rowData["Zip Code\n(Number only)"]) && !is_numeric($rowData["Zip Code\n(Number only)"])) {
            $rowErrors[] = '* Zip Code must be numeric';
        }
    
        if (!empty($rowData["Credit Limit\n(Accept Number/Decimal)"]) && !is_numeric($rowData["Credit Limit\n(Accept Number/Decimal)"])) {
