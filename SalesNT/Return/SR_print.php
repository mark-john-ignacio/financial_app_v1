...
if($lPrintPosted==0 && (int)$autopost==1){

	mysqli_query($con,"Update sales set lapproved=1, lprintposted=1 where compcode='$company' and ctranno='$csalesno'");
	
	mysqli_query($con,"INSERT INTO logfile(`ctranno`, `cuser`, `ddate`, `cevent`, `module`, `cmachine`, `cremarks`) 
		values('$csalesno','$preparedby',NOW(),'AUTO POST','SALES INVOICE','$compname','Post Record')");
	
	$status = "Posted";

?>
<script type="text/javascript">
		//Pag Posted .. Insert to AcctEntry
	
			$.ajax ({
				url: "../../include/th_toAcc.php",
				data: { tran: <?php echo $csalesno;?>, type: "SI" },
				async: false,
				success: function( data ) {
					if(data.trim()!="False"){
						
						//$("#AlertMsg").html("&nbsp;&nbsp;<b>" + <?php echo $csalesno;?> + ": </b> Successfully Posted...");
						//$("#alertbtnOK").show();
...
mysqli_query($con,"INSERT INTO logfile(`compcode`, `ctranno`, `cuser`, `ddate`, `cevent`, `module`, `cmachine`, `cremarks`) values('$company', '$csalesno','$preparedby',NOW(),'PRINTED','SALES INVOICE','$compname','Printed Record')");
