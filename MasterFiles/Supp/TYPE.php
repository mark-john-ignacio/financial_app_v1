	...
	<div>
        ...
        <div>
        	<div style="float:left; width:50%">
				<font size="+2"><u>Suppliers Master List > Types</u></font>	
            </div>
			...
			<table id="example" class="display" cellspacing="0" width="100%">
				...
				$company = $_SESSION['companyid'];
					...
					$sql = "select * from groupings where compcode='$company' and ctype='SUPTYP' order by cdesc";
...
?>
	...
	$(function(){
		...
		//alert(chkAccess('CUSTYPE_New.php'));
		
		// Adding new user
		$("#btnadd").on("click", function() {
			
		 var x = chkAccess('SUPTYPE_New');
		 ...
		 if(x.trim()=="True"){
			...
			$('#myModalLabel').html("<b>Add New Supplier Type</b>");
			$('#myModal').modal('show');
		 } else {
		...
		function setStat(code, stat){
			...
			if(x.trim()=="True"){				
				$.ajax ({
					...
					data: { code: code,  stat: stat, typz: 'SUPTYP' },
					async: false,
					success: function( data ) {
						if(data.trim()!="True"){
