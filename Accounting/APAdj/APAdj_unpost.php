...
<body style="padding:5px">
	<form action="APAdj_unpost_tran.php" name="frmunpost" id="frmunpost" method="POST">
		...
		<div>
			<section>
              ...
              <th>Customer</th>
					...
					<tbody>
					<?php
					$alrr = mysqli_query($con,"Select a.crefapcm from apv_deds a left join apv b on a.compcode=b.compcode and a.ctranno=b.ctranno where a.compcode='$company' and b.lcancelled=0");
					$refpos[] = "";
					while($rowxcv=mysqli_fetch_array($alrr, MYSQLI_ASSOC)){
						$refpos[] = $rowxcv['crefapcm'];
					}

					$result=mysqli_query($con,"select A.*, B.cname from apadjustment A left join suppliers B on A.compcode=B.compcode and A.ccode=B.ccode where A.compcode='$company' and a.ctranno not in ('".implode("','",$refpos)."') and (a.lapproved=1 or a.lcancelled=1)  order by A.ddate DESC");
