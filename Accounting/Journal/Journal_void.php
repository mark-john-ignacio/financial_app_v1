...
<body style="padding:5px">
	<form action="Journal_void_tran.php" name="frmunpost" id="frmunpost" method="POST">
		...
		<div>
			<section>
				...
				<table id="example" class="table table-hover " cellspacing="1" width="100%">
					...
					$result=mysqli_query($con,"select * from journal where compcode='$company' and (lapproved=1 and lvoid=0) order by ddate DESC");
            	...
            	<td><?php echo $row['cmemo'];?></td>
							<td align="right">
								<?php
								if (floatval($row['ntotdebit']) != floatval($row['ntotcredit'])) {
									echo "Unbalanced";
								}else{
									echo number_format($row['ntotdebit'],2);
								}
								?>
							</td>
