<?php
	...
	{
		$query = "SELECT B.mrp_jo_ctranno, A.ddatestart FROM mrp_jo_process_t A left join mrp_jo_process B on A.compcode=B.compcode and A.ctranno=B.ctranno where A.compcode='".$_SESSION['companyid']."' and B.mrp_jo_ctranno='".$tranno."' and A.ddateend IS NULL";
		$statement = $connect->prepare($query);
		$statement->execute();
		return $statement->rowCount();
	}
