...
<body>
	<?php
		$sqlhead = mysqli_query($con,"Select * from invcount where compcode='$company' and ctranno='".$_REQUEST['id']."'");
	...
	?>
		...
		<form id="frmCount" name="frmCount" method="post" action="<?="https://".$_SERVER['SERVER_NAME']?>/Inventory/Count/InvCnt_EditSave.php">
    	...
    	<input type="hidden" name="hdncancel" id="hdncancel" value="<?php echo $lCancelled;?>">
			...
			<fieldset>
				<legend><div class="col-xs-6 nopadding">Inventory Count Details</div>
				...
				<div class="col-xs-12 nopadwtop">
					...
					<div class="col-xs-2 nopadding">
						<select class="form-control input-sm" name="seltempname" id="seltempname">		 
							<option value="" <?=($seltemid=="") ? "selected" : ""?>>None</option>			
							<?php
								foreach($arrtempsname as $xcfv){
									if($xcfv['section_nid']==$selwh){
							?>	
				...
				<div class="col-xs-12 nopadwtop">
					...
					<div class="col-xs-3 nopadding">
						<select class="form-control input-sm" name="selcntyp" id="selcntyp">		
							<option value="ending" <?=($seltype=="ending") ? "selected" : ""?>>Inventory Ending</option>		
							<option value="output" <?=($seltype=="output") ? "selected" : ""?>>Production Output</option>							
						</select>
					</div>
			...
			<div class="alt2" dir="ltr" style="
						...
						overflow: auto">
									...
									<table name='MyTbl' id='MyTbl' class="table table-scroll table-striped table-condensed">
										...
										<tbody>
											<?php
												$sqlhead = mysqli_query($con,"Select A.*, B.citemdesc from invcount_t A left join items B on A.compcode=B.compcode and A.citemno=B.cpartno where A.compcode='$company' and A.ctranno='".$_REQUEST['id']."'");
												if (mysqli_num_rows($sqlhead)!=0) {
											...
											?>
												<tr>				
													...
													<td>
														<input type='text' class="numeric form-control input-xs text-center" name="txtnqty" id="txtnqty<?=$cnt?>" value="<?=number_format($row['nqty'],2)?>">
													</td>
													<td align="center"><button type="button" class="btn btn-danger btn-xs" id="btnDel" id="btnDel"><i class="fa fa-times"></i></button></td>
												</tr>
