...
<body style="padding:5px">
	<form action="ARAdj_unpost_tran.php" name="frmunpost" id="frmunpost" method="POST">
		...
		<div>
			<section>
				...
				<table id="example" class="table table-hover " cellspacing="1" width="100%">
					...
					<tbody>
					<?php
					$alrr = mysqli_query($con,"Select a.aradjustment_ctranno from receipt_deds a left join receipt b on a.compcode=b.compcode and a.ctranno=b.ctranno where a.compcode='$company' and b.lcancelled=0");
					$refpos[] = "";
					while($rowxcv=mysqli_fetch_array($alrr, MYSQLI_ASSOC)){
						$refpos[] = $rowxcv['aradjustment_ctranno'];
					}
					
					$result=mysqli_query($con,"select a.*,IFNULL(b.ctradename,b.cname) as cname from aradjustment a left join customers b on a.compcode=b.compcode and a.ccode=b.cempid where a.compcode='$company' and a.ctranno not in ('".implode("','",$refpos)."') and (a.lapproved=1 or a.lcancelled=1) order by a.ddate desc");
