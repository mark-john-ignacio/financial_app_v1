...
<body style="padding:5px">
	<form action="OR_void_tran.php" name="frmunpost" id="frmunpost" method="POST">
	
		<div>
			<section>
					<div>
						<div style="float:left; width:50%">
							<font size="+2"><u>Receive Payment List</u></font>	
						</div>
					</div>
				<br><br>
				...
				<table id="example" class="table table-hover " cellspacing="1" width="100%">
					<thead>
						<tr>
							...
							<th class="text-center">OR Series</th>
					...
					<tbody>
					<?php
					$alrr = mysqli_query($con,"Select a.corno from deposit_t a left join deposit b on a.compcode=b.compcode and a.ctranno=b.ctranno where a.compcode='$company' and b.lcancelled=0 and b.lvoid=0");
					$refpos[] = "";
					while($rowxcv=mysqli_fetch_array($alrr, MYSQLI_ASSOC)){
						$refpos[] = $rowxcv['corno'];
					}
					
					$result=mysqli_query($con,"select a.*,IFNULL(b.ctradename,b.cname) as cname from receipt a left join customers b on a.compcode=b.compcode and a.ccode=b.cempid where a.compcode='$company' and a.ctranno not in ('".implode("','",$refpos)."') and (a.lapproved=1 and a.lvoid=0) order by a.ddate desc");
					...
					<!-- <a href="javascript:;" onClick="printchk('<?//php echo $row['ctranno'];?>');"> </a>-->
						<tr>
							...
							<td style="text-align: right"><?php echo number_format($row['napplied'],2);?></td>
						</tr>
					<?php 
					}				
					mysqli_close($con);				
