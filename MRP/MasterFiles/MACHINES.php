	...
	<div>
      ...
      <div>
        <div style="float:left; width:50%">
					<font size="+2"><u>Machines List	</u></font>	
        </div>            
      </div>
			...
			<table id="example" class="display" cellspacing="0" width="100%">
				...
				<tbody>
					<?php
						...
						$sql = "select * from mrp_machines where compcode='$company'order by cdesc";							
	...
	<script>
		...
		$(function(){
			...
			$("#btnadd").on("click", function() {
				...
				if(x.trim()=="True"){
					...
					$('#myModalLabel').html("<b>Add New Machine</b>");
					$('#myModal').modal('show');
				} else {
			...
			$("#btnSave, #btnUpdate").on("click", function() {
				...
				if(vardesc==""){
					...
					if(xcstat=="False"){
						...
						$("#add_err").html("<center><b>Machine Name Already Exist!</b></center>");
						$("#add_err").show();
					}
		...
		function editgrp(code,desc){
			...
			if(x.trim()=="True"){
				...
				$('#myModalLabel').html("<b>Update Machine Details</b>");
				$('#myModal').modal('show');
			} else {
				$("#AlertMsg").html("<center><b>ACCESS DENIED!</b></center>");
				$("#AlertModal").modal('show');
