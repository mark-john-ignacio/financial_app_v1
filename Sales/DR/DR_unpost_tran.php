<?php
	...
	$status = "True";
			...
			if (!mysqli_query($con,"Update dr set lapproved=0,lcancelled=0 where compcode='$company' and ctranno in ('".implode("','",$_POST["allbox"])."')")){
				$status = "False";	
			}else{
				...
				foreach($_POST["allbox"] as $rz){
					...
					values('$rz','$preparedby',NOW(),'UNPOST','DELIVERY RECEIPT','$compname','UnPost Record')");
...
?>
				...
				<script>
					...
					window.location.href="DR_unpost.php";
				</script>
<?php
			}else{
?>
