...
$_SESSION['pageid'] = "InvCnt_edit.php";
                  ...
                  <table name='MyTbl' id='MyTbl' class="table table-scroll table-striped table-condensed">
                   <thead>
                    ...
                    <tbody>
                    	...
                    	<?php
							...
							$cnt = $cnt + 1;
								...
								foreach($rowdetloc as $localocs){
									if(intval($localocs['nid']) == intval($rowdet['nlocation'])){
										$isslcdt = "selected";
									}else{
										$isslcdt = "";
									}
									$opttxts = $opttxts."<option value=\"". $localocs['nid']."\" ".$isslcdt.">".$localocs['cdesc']."</option>";
						...
						?>
                    	<tr>    
                            <td><div id="rowItmNum"><?php echo $cnt; ?></div></td>
                            <td><input type='hidden' value='<?php echo $rowdet['citemno'];?>' name='txtcpartno'><?php echo $rowdet['citemno'];?></td>
                            ...
                            <td><input type='text' value='<?php echo $rowdet['nqty'];?>' name='txtnqty' class="numeric form-control input-xs" style="align: right" autocomplete="false"></td>
                            <td><input type='text' value='<?php echo $rowdet['cbarcode'];?>' name='txtbcode' class="form-control input-xs" style="align: right" autocomplete="false"></td>
                            <td><input type='text' value='<?php echo $rowdet['cserial'];?>' name='txtserial' class="form-control input-xs" style="align: right" autocomplete="false"></td>
                            <td><select name='sellocs' class="form-control input-xs"><?php echo $opttxts; ?></select></td>
                            <td><div style="position: relative"><input type="text" name="txtexpdte" class="datepick form-control input-xs" value='<?php echo date_format(date_create($rowdet['dexpdte']),"m/d/Y");?>'></div></td>
                            <td><input class='btn btn-danger btn-xs' type='button' id='<?php echo $rowdet['citemno'];?>_delete' value='delete' onClick='deleteRow(this);' /></td>
                       </tr>
                            <script>
											$("#del<?php echo $rowdet['citemno'];?>").on("click", function() {
												//alert("HELLO");
		...
		afterSelect: function(item) { 	
			...
			if(exist == "NO"){
				...
				//alert(varlocs);
					...
					var trRow4 = "<td><input type='text' value='1' name='txtnqty' class=\"numeric form-control input-xs\" style=\"align: right\" autocomplete=\"false\">"+"</td>";
					...
					$("<tr>"+trRow0+trRow1+trRow2+trRow3+trRow4+trRow5+trRow6+trRow7+trRow8+trRow9+"</tr>").prependTo("#MyTbl > tbody");		
		...
		if(trancode!=""){
			$("#MyTbl > tbody > tr").each(function(index) {
				...
				$.ajax ({
					...
					data: { trancode: trancode, indx: index, citmno: citmno, cunit: cunit, nqty:nqty, bcode:bcode, cserial:cserial, selloc:selloc, dexpdte:dexpdte },
					async: false,
					success: function( data ) {
						if(data.trim()!="False"){
